...
Vercel does **not** wait for GitHub checks before deploying, and nothing else gates a deploy on
CI either. See "Branch rulesets" below for what the repo does enforce, and why required status
checks were deliberately left off.

### Branch rulesets

Two repository rulesets are active, one per branch: `protect-main` and `protect-release`. Each
enforces exactly two rules, and neither has a bypass list:

- `deletion` — the branch cannot be deleted.
- `non_fast_forward` — the branch cannot be force-pushed.

That is all of it. `verify` is deliberately **not** a required status check on either branch. A
required check blocks direct pushes as well as merges: a fresh merge commit has no check results
yet, so `git merge main && git push` into `release` would be rejected outright. Requiring the check
would therefore force every change through a pull request, which is not how this repo ships (see
"Day-to-day flow" below). CI still runs on every push and PR — it reports, it just does not gate.

Force-push protection is the rule that earns its keep on `release`: a rewritten history there
desyncs `.github/.release-please-manifest.json` from the real git tags, and release-please would
then compute the next version from commits that no longer exist.

Neither rule affects the release bot — it only ever merges a PR and pushes a tag — so the GitHub
Actions app needs no bypass entry. That changes if you ever do require `verify` on `release`; see
the gotchas under Versioning.

Check what is actually in force (not just what exists) with:
...
release-please can only deliver that commit as a pull request, and nobody needs to review a version
number, so the workflow merges the PR itself in the same run and then calls release-please a second
...
- The bump PR is created and merged with the default `GITHUB_TOKEN`, so GitHub runs no workflows
  for it — no CI on the PR, and no CI or Release run for the merge push. That's fine: it only ever
  touches version fields and the changelog, and the tag is cut inside the same run. Vercel does
  still deploy the merge push (its webhook is not subject to that rule).
- If you add branch protection to `release` that requires the `verify` check, the bot's merge will
  be blocked, because the PR never gets a check. Use a repository ruleset and put the GitHub Actions
  app on its bypass list, or require the check on `main` only. The rulesets in place today require
  no checks at all, so this does not bite — see "Branch rulesets" above before changing that.
- `CHANGELOG.md` is generated by release-please and listed in `.prettierignore`, so `verify` does
  not fight its formatting. Do not hand-edit it.
...
2. When ready to ship, merge `main` into `release` and push. CI runs; Vercel deploys; the Release
   workflow merges the version bump and creates the tag and GitHub Release on its own.
3. Merge `release` back into `main`.
...
- Create the two branch rulesets described under "Branch rulesets" — deletion and force-push
  blocked on `main` and `release`, no required status checks.
